 ...
 try {
     console.log(FormValues)
        const res = await axios.patch('https://future-medical.herokuapp.com/admin/edit',
        {
              ...
              admin_profilePic:token.profilePic,
...
//مستشفى الراعى الصالح        
    ...
    const initial_state = {username:token.username , contactnumber : token.entity.telephone[0], entityname : token.entity.name,entityaddress: token.entity.address[0],email:token.email , entitynamearabic:token.entity.arabic_name }
  ...
  <div className="container">       
        ...
        <div className="card-header bg-transparent">
          ...
          <div className="card-body">
            ...
            {edit ? ( <input   className='input-form' type="number" onChange={(e)=>handlechange(e)} value={FormValues.contactnumber} name="contactnumber" placeholder="Enter Contact Number"/>) :<span> {token.entity.telephone[0]} </span>}  
            ...
            {edit ? ( <input  className='input-form' type="text" onChange={(e)=>handlechange(e)} value={FormValues.entityname} name="entityname" placeholder="Enter Entity name"/>) : <span> {token.entity.name} </span>}
            ...
            {edit ? ( <input  className='input-form' type="text" onChange={(e)=>handlechange(e)} value={FormValues.entitynamearabic} name="entitynamearabic" placeholder="Enter Entity name"/>) : <span> {token.entity.arabic_name} </span>}
            ...
            {edit ? ( <input  className='input-form' type="text" onChange={(e)=>handlechange(e)} value={FormValues.entityaddress} name="entityaddress" placeholder="Enter Entity Address"/>) : <span> {token.entity.address[0]} </span>}   
            ...
            {edit ? ( <input className='input-form' type="text" onChange={(e)=>handlechange(e)} value={FormValues.email} name="email" placeholder="Enter Admin email"/>) : <span> {token.email}</span>}       
