...
export default function Hospitals() {
  ...
  const Get_Hospitals_Api = async () => {
    try {
      const res = await axios.get(
        "https://future-medical.herokuapp.com/hospitals"
      );
      const data = await res.data;
      let i = 1;
      hospitals_list = [];
  ...
  const Get_Hospitals_Deactivated_Api = async (activateList) => {
    ...
    try {
      const res = await axios.get(
        `https://future-medical.herokuapp.com/admin/hospitals/deactivated`,
        {
          headers: {
            Authorization: `Bearer ${token.token}`,
          },
      ...
      >
        ...
        {!viewadd && (
          <AddHospital
            changeadd={changeadd}
            goback={goback}
            gethospitals={Get_Hospitals_Api}
          />
        )}
        {alert_delete && (
          <AlertDelete
